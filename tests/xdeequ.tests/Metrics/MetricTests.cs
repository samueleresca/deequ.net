...
namespace xdeequ.tests.Metrics
{
    public class MetricTests
    {
        ...
        {
            var metric = new DoubleMetric(Entity.Column, "metric-name", "instance-name", Try<double>.From(() => 50));
        ...
        {
            var metric = new DoubleMetric(Entity.Column, "metric-name", "instance-name", Try<double>.From(() => throw sampleException));
            metric.Flatten().ShouldBe(new List<DoubleMetric> { metric });
        }
        ...
        {
            var distribution = new Distribution(new Dictionary<string, DistributionValue>
            {
                {"a", new DistributionValue(6, .6)},
                {"b", new DistributionValue(4, .4)}
            }, 2);

            var metric = new HistogramMetric("instance-name", Try<Distribution>.From(() => distribution));

            var expected = new List<DoubleMetric>
            {
        ...
        {
            var metric = new HistogramMetric("instance-name", Try<Distribution>.From(() => throw sampleException));
            var expected = new List<DoubleMetric>
            {
                new DoubleMetric(Entity.Column, "Histogram.bins", "instance-name", Try<double>.From(()=>throw sampleException)),
            };
            metric.Flatten().Any(x => x.Name == "Histogram.bins").ShouldBeTrue();
