    ...
    public class InMemoryMetricsRepositoryTests
    {
        ...
        public void save_and_retrieve_AnalyzerContexts()
        {
            ...
            Evaluate(_session, (context, repository) =>
            {
                ...
                var loadResults = repository.LoadByKey(resultKey).Value;

                var loadedResultsAsDataFrame =
                    loadResults.SuccessMetricsAsDataFrame(_session, Enumerable.Empty<IAnalyzer<IMetric>>());

                var resultAsDataFrame =
        ...
        {
            var metrics = new Dictionary<IAnalyzer<IMetric>, IMetric>
            {
                {
            ...
            var resultsWithMixedValues = new AnalyzerContext(metrics);
            var successMetrics = resultsWithMixedValues.MetricMap.Where(
                x =>
                {
                    var dm = x.Value as DoubleMetric;
                    return dm.Value.IsSuccess;
                });

            var resultsWithSuccessfulValues =
            ...
            var loadedAnalyzerContext = repository.LoadByKey(resultKey).Value;
