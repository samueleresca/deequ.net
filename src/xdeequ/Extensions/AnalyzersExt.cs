...
namespace xdeequ.Extensions
{
    public static class AnalyzersExt
    {
        ...
        public static Option<S> Merge<S>(Option<S> state, Option<S> anotherState) where S : State<S>
        {
            ...
            {
                var state = (stateA.HasValue, stateB.HasValue) switch
                {
        ...
        {
            return DoubleMetric.Create(entity, name, instance, value.Get());
        }

        public static Column ConditionalSelection(Column selection, Option<string> condition)
        {
            var conditionColumn = condition.Select(cond => Expr(cond));
            return ConditionalSelectionFromColumns(selection, conditionColumn);
        }

        public static bool HasColumn(StructType schema, string column)
        {
            return schema.Fields.Select(x => x.Name).Contains(column, StringComparer.InvariantCultureIgnoreCase);
        }
        ...
        public static Action<StructType> IsNotNested(string column)
        {
            return schema =>
            {
                if (!HasColumn(schema, column)) return;
        ...
        {
            var structFields = schema.Fields.First(x => x.Name == column);
            return structFields;
        }
        ...
        {
            return enumerable.Count() == 1 ? Entity.Column : Entity.MultiColumn;
        }
    }
}